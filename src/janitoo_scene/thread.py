...
def make_thread(options, force=False):
    if get_option_autostart(options, OID) or force:
        return SceneThread(options)
    else:
        return None

class SceneThread(JNTBusThread):
    """The scene thread
    ...
    def init_bus(self):
        """Build the bus
        """
        from janitoo_scene.bus import SceneBus
        self.section = OID
        self.bus = SceneBus(options=self.options, oid=self.section, name='Scene Manager bus')
