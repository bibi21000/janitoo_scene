...
def make_thread(options):
    if get_option_autostart(options, 'events') == True:
        return ScenariosThread(options)
    else:
        return None

class ScenariosThread(JNTBusThread):
    """The Hardware thread
    ...
    def init_bus(self):
        ...
        """
        self.section = 'events'
        self.bus = ScenariosBus(options=self.options, oid=self.section, name='Event Manager bus', product_name="Event controller", product_type="Core thread")

class ScenariosBus(JNTBus):
